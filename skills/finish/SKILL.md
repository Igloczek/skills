...
1. Read the PR, linked issue/spec, review-persona selection and results, checks,
...
- Do not report readiness until `review-standard` and every selected specialist
  lane have terminal results. Require an explicit `NOT_RUN` reason for each
  unselected specialist and include domain-expert results when domain rules were
  involved.
