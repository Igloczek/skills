...
function main(): void {
  ...
  for (const dependency of manifest) {
    ...
    if (typeof entry.name !== 'string' || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(entry.name)
      ...
      || typeof entry.group !== 'string' || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(entry.group)
      || (entry.skill !== undefined && (typeof entry.skill !== 'string' || !entry.skill.trim()))
      || !Array.isArray(entry.usedBy) || entry.usedBy.length === 0
      || entry.usedBy.some(consumer => typeof consumer !== 'string' || !skills.has(consumer))) {
      fail('external skill manifest contains an invalid entry');
    }
    const name = entry.name;
    if (dependencyNames.has(name)) fail(`external skill manifest has duplicate ${name}`);
    dependencyNames.add(name);
    const selectedSkill = typeof entry.skill === 'string' ? entry.skill : name;
    for (const consumer of entry.usedBy as string[]) {
      if (!skills.get(consumer)!.body.includes(`\`${selectedSkill}\``)) {
        fail(`${consumer} does not invoke configured companion ${selectedSkill}`);
      }
    }
  }
