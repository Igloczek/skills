...
Owner: `shape`, `specify`, and `build`. Keep briefs, short-horizon specs,
terminal states, and compact handoff markers stable enough for the next
workflow to consume.

## change-executor

Owner: `build` and `fix`. Work in an isolated branch/worktree, take small
feedback-driven steps, reset an unproductive context trajectory, run the
configured validation, and leave a reviewable branch or PR.
